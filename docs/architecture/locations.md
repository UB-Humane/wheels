...
- "Mark Distributed" is available to the entity that submitted the request (distribution users for distribution-submitted requests — shown on the distribution's own Bike Tickets dashboard only; production users for production-submitted requests — shown on the production's Delivery dashboard, to the left of "Back to Taken Up", only when the request has no distribution) *or* to a production `admin`/`master_mechanic` regardless of who submitted it — needed since a request submitted "on behalf of a distribution" (above) may have no distribution-side user actually managing it. The redirect after marking distributed goes to the distribution's own tickets page only if the acting user actually belongs to that distribution; otherwise it goes to the production's.
