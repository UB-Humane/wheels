...
- Routes:
  ...
  - `GET /productions/:id/delivery` — delivery dashboard (ready_for_delivery/taken_up/delivered/distributed)
...
- "Mark Distributed" is only available to the entity that submitted the request (`requestor_for?`) — distribution users for distribution-submitted requests, shown on the distribution's own Bike Tickets dashboard only; production users for production-submitted requests, shown on the production's Delivery dashboard, to the left of "Back to Taken Up", only when the request has no distribution. A production `admin`/`master_mechanic` cannot mark a distribution-submitted request distributed just by virtue of their role — including one submitted "on behalf of a distribution" (above); that distribution's own users are the only ones who can close it out, even though they didn't submit it themselves. The redirect after marking distributed goes to the distribution's own tickets page for a distribution-submitted request, or the production's Delivery dashboard otherwise — based on whether the request has a distribution, not on the acting user's own memberships (a superadmin closing out a distribution's request still lands on that distribution's page).
