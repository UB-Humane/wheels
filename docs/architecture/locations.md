...
- `printed` is set to `true` by `PATCH /bike_requests/:id/mark_printed` (`BikeRequestsController#mark_printed`, gated by `authorized_for_production?` — any production member, matching who can see the "Print All"/"Print Overview" buttons), fired automatically from `app/javascript/controllers/print_controller.js`'s `printLabels` (the "Print All" button) once the print popup has opened. `printCard` ("Print Overview") does not set it — only printing the actual labels counts. There's no manual "unprint" action; it's only ever reset by `back_to_requested` (below), since a request bounced back to square one needs its labels reprinted. A printed card shows a green border (in place of the default gray one — denied/overdue red still takes priority) and a "PRINTED" badge next to OVERDUE/Due date; the JS applies both directly to the DOM on a successful response, without a page reload, in addition to the same being server-rendered on next load.
