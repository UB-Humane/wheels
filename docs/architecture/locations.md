...
Model: `BikeRequest` — fields: `codename`, `due_date`, `status` (enum), `denial_reason`, `status_before_archival`, `owner_id` (FK to users — the production member assigned at approval), `taker_id` (FK to users — the production member who claimed the delivery via "I've got this"), `printed` (boolean, default `false`). No separate requestor name/phone is collected — `user` (the submitting account, set from `current_user` at creation) already has `name`/`mobile_number`, so that's what's shown wherever a requestor's identity is needed (card and the `printCard` overview read `req.user.name`/`req.user.mobile_number`, styled "Requested by {name} ({number})"; the per-bike `printLabels` output omits requestor/owner entirely to keep labels compact). The card and `printCard` overview show the same for `owner` when present ("Owned by {name} ({number})").
...
Status flow: `requested (1)` → production approves → `pending (0)` → admin/master mechanic marks ready → `ready_for_delivery (2)` → delivery volunteer takes it up → `taken_up (7)` → taker/owner/admin/master mechanic marks done → `delivered (3)` → requestor marks distributed → `distributed (4)`. Production can also deny: `requested` → `denied (5)` → distribution edits and resubmits → `requested`.
...
- An `admin` or `master_mechanic` (or superadmin) can advance `pending` → `ready_for_delivery` via the "Ready for Delivery" button (`BikeRequestsController#complete_all`, gated by `authorized_for_status_correction?`) — not open to plain volunteers.
...
- `taken_up` → `delivered` ("I'm done!") is restricted to the recorded `taker`, the `owner`, or an `admin`/`master_mechanic` (or superadmin) — not open to any production member (`authorized_to_complete_delivery?`); the button itself is hidden in the UI for anyone outside that set, rather than shown and then rejected. Setting `delivered` from any *other* status (a skip-ahead from `ready_for_delivery`, or the `distributed` → `delivered` correction below) requires `admin` or `master_mechanic` instead — there is no way for a plain volunteer to set `delivered` directly.
- "Mark Distributed" is only available to the entity that submitted the request (`requestor_for?`) — distribution users for distribution-submitted requests, shown on the distribution's own Bike Tickets dashboard only; production users for production-submitted requests, shown on the production's Delivery dashboard, to the left of "Back to Taken Up", only when the request has no distribution. A production `admin`/`master_mechanic` cannot mark a distribution-submitted request distributed just by virtue of their role — including one submitted "on behalf of a distribution" (above); that distribution's own users are the only ones who can close it out, even though they didn't submit it themselves. The redirect after marking distributed goes to the distribution's own tickets page for a distribution-submitted request (based on whether the request has a distribution, not on the acting user's own memberships — a superadmin closing out a distribution's request still lands on that distribution's page), or the production's Delivery dashboard otherwise — `distributed` isn't a tab there (see below), so a production-submitted request has no dedicated view of its own once distributed.
...
- Any move to `requested` — `back_to_requested` (`pending` → `requested`) or a distribution's resubmit (`denied` → `requested`) — clears both `owner_id` and `taker_id`, since neither an owner nor a taker is meaningful before the request is re-approved. `back_to_requested` additionally clears `printed`, since a request needs its labels reprinted once it's back to square one (a `denied` request was never printed in the first place — print buttons only show once a request has left `requested` — so the resubmit path has nothing to clear).
- `printed` is set to `true` by `PATCH /bike_requests/:id/mark_printed` (`BikeRequestsController#mark_printed`, gated by `authorized_for_production?` — any production member, matching who can see the "Print Stickers"/"Print Overview" buttons), fired automatically from `app/javascript/controllers/print_controller.js`'s `printLabels` (the "Print Stickers" button) once the print popup has opened. `printCard` ("Print Overview") does not set it — only printing the actual labels counts. There's no manual "unprint" action; it's only ever reset by `back_to_requested` (below), since a request bounced back to square one needs its labels reprinted. A printed card shows a green border (in place of the default gray one — denied/overdue red still takes priority) and a "PRINTED" badge next to OVERDUE/Due date; the JS applies both directly to the DOM on a successful response, without a page reload, in addition to the same being server-rendered on next load.
- Push notifications (`docs/architecture/notifications.md`) fire for a focused subset of the flow, not every transition: a genuine distribution-submitted `create` (`BikeRequestsController#notify_new_request`, not the production self-request or on-behalf-of-a-distribution paths, since those skip approval entirely) notifies that production's `admin`/`master_mechanic` users; `approve`, `deny`, and `delivered` (`BikeRequestsController#notify_status_change`) each notify the request's `user` (the original submitter). `ready_for_delivery` (fired from `complete_all`, the actual "Ready for Delivery" button's action — not the same-named `update` status param, which the UI never sends) notifies both the submitter and the `owner`, since the owner is the production member responsible for the request at that point and may not be the same person who submitted it. `taken_up`, archive/unarchive, and all back-transitions stay silent.

Production's Bike Tickets dashboard only shows `requested`/`pending`/`archived` — once a request reaches `ready_for_delivery`, it moves to the production's Delivery dashboard instead, covering `ready_for_delivery`/`taken_up`/`delivered` (`BikeRequest::DELIVERY_STATUSES`). Distribution's Bike Tickets dashboard is unaffected and still shows the full flow end to end (`requested`/`pending`/`ready_for_delivery`/`delivered`/`distributed`/`archived` — `taken_up` is Delivery-exclusive and never shown there).
